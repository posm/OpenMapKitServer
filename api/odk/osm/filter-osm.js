...
filter.file = function(filePath, filterObj, cb) {
  ...
  if (filterObj.blacklist) {
    // if a blacklist for this form exists and the filename is found in the
    // blacklist, filter it.
  ...
  if (unsubmitted || conflict) {
    fs.readdir(pathDir, function(err, files) {
      ...
      // otherwise, it is submitted, and we don't want it
      cb(filePath, false);
    });
    return;
  }
  ...
  if (typeof filterObj.submitTimeStart === 'string' ||
      typeof filterObj.submitTimeEnd === 'string'
    ) {
